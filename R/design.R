...
is_valid_df_desc <- function (df_desc) {
	...
	if (is.list(d$style)) {
		...
		if (!is.null(s$rm_empty_cols) && !is.logical(s$rm_empty_cols)) {
			message <-
				paste0(
					"rm_empty_cols element of style component is ",
					typeof(s$rm_empty_cols),
					" but must be logical or NULL."
				)
			return(data.frame(valid = FALSE, message))
		}
	}
