...
If you are not sure which attributes are available or where they are located in the resource, it can be helpful to start by extracting all available attributes. If you are more comfortable with xml, you can also use `xml2::xml_structure` on one of the bundles from you bundle list, this will print the complete xml structure into you console. Then you can get an overview over the available attributes and their location and continue by doing a second, more targeted extraction to get your final data frame.
...
Internally, `fhir_crack()` will always complete the design you provided so that it contains `resource`, `cols` and `style` with its elements `sep`, `brackets` and `rm_empty_cols`, even if you left out `cols` and `style` completely. You can tell `fhir_crack()` to return the completed design together with the resulting data.frames by setting the argument `return_design = T`:

```{r}
tables_with_design <- fhir_crack(bundles = patient_bundles, 
								 design = design1, verbose = 0, 
								 return_design = TRUE)

tables_with_design$design
```
...
Consider the following example where we want to download MedicationStatements referring to a certain medication we specify with its snomed code and also the Patient resources these MedicationStatements are linked to.
...
df1 <- fhir_crack(bundle_list, design1, verbose = 0)
df1$Patients
```
...
df2 <- fhir_crack(bundle_list, design2, verbose = 0)
df2$Patients
```
