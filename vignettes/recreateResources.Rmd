...
We can create a list of such a bundles from a cast table using the function `fhir_build_bundles`, which takes a cast table and the resource type represented in the table, as well as information on the type of bundle you want to create and the number of resources (`bundle_size`) that should be bundled together:
...
It is also possible to upload a bundle of resources together. The bundles in the `fhir_bundle_list()` we've created with `build_bundles()` are such bundles:
