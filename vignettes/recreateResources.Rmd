...
It is also possible to bundle together several resources to upload them to the server together. This is done using bundles of type transaction or batch (see https://www.hl7.org/fhir/bundle.html and https://www.hl7.org/fhir/http.html). 
...
```{r}
bundle <- fhir_build_bundle(modified_table, 
...
```{r}
bundle <- fhir_build_bundle(request_table,
                            resource_type = "Patient", 
                            bundle_type = "transaction", 
                            verbose = 0)
```
...
```{r}
#create resource
new_resource_with_id <- fhir_build_resource(cast_table[1,], resource_type = "Patient")
...
tables <- fhir_crack(bundles = bundles, design = fhir_design(Patient, Observation), verbose = 0)

#cast
cast_tables <- lapply(tables, fhir_cast, brackets = c("[","]"), sep = ":::", verbose = 0)

```
The cast tables look like this:
...
cast_tables$Patient$request.method <- "PUT"
cast_tables$Patient$request.url <- paste0("Patient/", cast_tables$Patient$id1)

#add request info to Observation
cast_tables$Observation$request.method <- "PUT"
cast_tables$Observation$request.url <- paste0("Observation/", cast_tables$Observation$id1)
```
...
```{r}
bundle <- fhir_build_bundle(cast_table = cast_tables)
```
