...
To create resources from this data, the `fhircrackr` makes use of the structure information inherent in the column names. If you want to get an overview over this structure before creating the actual xml-objects, you can use the function `fhir_tree()` that creates a string reprenting the structure wich can be printed to the console using `cat()` or written to a text file:
...
As you can see, this example Medication has ingredient elements which have an `id` attribute. `fhir_crack()` will just extract all kinds of attributes, e.g. from this bundle containing the above Medication resource:
...
```{r, eval=FALSE}
fhir_put(url = "http://hapi.fhir.org/baseR4/Patient/id1", body = new_resource_with_id, brackets = c("[", "]"))
```
```{r, echo=FALSE}
