...
fhir_resource_type("nonExistentResource") #warning
```
...
```{r}
request1 <- fhir_url(url = "http://hapi.fhir.org/baseR4",
					resource = "Patient",
					parameters = list("code" = "http://loinc.org|1751-1")
		 			)

request1

request2 <- fhir_url(url = "http://hapi.fhir.org/baseR4",
					resource = "Patient",
					parameters = list("birthdate" = "lt2000-01-01",
								      "code" = "http://loinc.org|1751-1")
		 			)

#request2 is to long to print here
...
Whenever you call `fhir_url()` or `fhir_search()`, the corresponding FHIR search request will be saved implicitly and can be accessed like this:
...
```{r, eval=F}
medication_bundles <- fhir_search(request = request, max_bundles = 3, log_errors = "myErrorFile")
```
