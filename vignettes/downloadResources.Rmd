...
- Downloading random samples from a server
...
Sometimes it can be useful to download a random sample of resources from a server, e.g. in some kind of case-control scenario. The fhircrackr offers a function `fhir_sample_resources()` which takes a base url, a resource type and (optionally) some FHIR Search parameters and returns a random sample of those resources with a given size.
...
pat <- fhir_table_description(resource = "Patient",
							  cols = c("gender", "birthDate"))


d <- fhir_crack(bundles = bundle, design = pat)
``` 
