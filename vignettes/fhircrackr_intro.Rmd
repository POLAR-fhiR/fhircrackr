...
All five style elements of can also be controlled directly by the `fhir_crack()` arguments `sep`, `brackets`, `remove_empty_columns`, `format` and `keep_attr`. If the function arguments are `NULL` (their default), the values provided as style arguments are used, if they are not NULL, they will overwrite any values in `fhir_table_description`. If both the function arguments and the `fhir_table_description` components are NULL, default values(`sep = " "`, `brackets = NULL`, `rm_empty_cols = TRUE`, `format` = 'compact', `keep_attr` = FALSE) will be assumed. 
