...
# only completely filled columns are shown here
list_of_tables$Patients[1:5,sapply(list_of_tables$Patients[1:5,], function(x) !any(is.na(x)))]
```
...
```
Note!  
To get `fhir_melt()` work, the data frame has to be *cracked with indices*.  
...
Since `fhir_crack()` ignores data like resources or data fields, that are not specified in `design`, it makes sense to store the original search result for reproducibility and in case you realize later on that you need elements from the resources that you haven't extracted at first.
