...
#### The style arguments
The remaining elements of a `fhir_table_description` are more or less style parameters. These elements control how `fhir_crack()` deals with multiple entries to the same element and with columns that are completely empty, i.e. have only `NA` values. Furthermore you can select the shape of the output tables by a format argument and control automatically build column names. In the outputs above you can see, that the style takes some default values if you skip it in `fhir_table_description()`.
  ...
  - The `format` element is a character either `compact` or `wide` that specifies the shape of the output table. For `compact` multiple entries are written in one cell separated by the separator `sep`. In the `wide` case for every entry a singel column will be created.  
  - The `keep_attr` flag is a logical the ask for keeping XML-Tag-Attributes as ending of the column names or not. e.g. `name/given@value` would result either in `name.given` or `name.given.value`.  
...
A fully default set of a `Patient` table description would be like this:  
```{r}
ftd <- fhir_table_description(
	resource = "Patient",
	cols     = list(
		gender = "gender",
		name   = "name/family",
...
If the data frame produced by `fhir_crack()` contains multiple entries, you'll probably want to divide these entries into distinct observations at some point. fhircrackr gives you two options to do this: `fhir_melt()` and `fhir_cast()`. The former spreads the entries across rows, creating a long format, the latter spreads them across columns, creating a wide format.
...
### Cast data frames with multiple entries
Instead of spreading the entries across rows, you can also spread them across columns using `fhir_cast()`. This function takes a data.frame/data.table with multiple entries and the brackets and separator that have been used in `fhir_crack()` as input:
...
The tables produced by `fhir_cast()` can also be used to recreate the resources that were cracked in the first place, as you'll the in the vignette about recreation of resources.
